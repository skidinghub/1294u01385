...
const mainIndex = randomIndex;
const isSubPath = window.location.pathname !== '/' && window.location.pathname.length > 1;
const videoPath = isSubPath
  ? `../bg/bg${mainIndex}.webm`
  : `bg/bg${mainIndex}.webm`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioMessages = [
    "/temeria Public Sunucum", "/marino Reklam Sunucum", "/athen Priw", "/susku Priw"
  ];
  let bioText = '';
  let bioIndex = 0;
